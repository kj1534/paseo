...
Pi tree navigation is exposed to app/web as a provider slash command. `/tree` returns selectable JSONL entries, and `/tree <entryId>` runs out-of-band through a temporary Paseo extension loaded into the Pi RPC process. The extension calls Pi's native `ctx.navigateTree(..., { summarize: false })`, then Paseo rehydrates the timeline and appends a local-only notice. This selection is a pending in-memory branch point until the next normal user message creates a new Pi JSONL entry; if the user reloads the agent before sending that message, Paseo may resume Pi's persisted leaf and clear the pending selection.
